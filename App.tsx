...
const generatePDF = (data: AIResponse) => {
  const doc = new jsPDF();

  // Professional Header Function (Black background)
  const addHeader = () => {
    ...
    doc.text('GEO-NARRATOR AI', 105, 10, { align: 'center' });

    doc.setTextColor(255, 255, 255); // White
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Cultural & Historical Insights | Educational Resource', 105, 17, { align: 'center' });
    doc.setFontSize(7);
    doc.text('Developer: Ambuj Kumar Tripathi', 105, 22, { align: 'center' });
  };

  // Professional Footer Function (Black background with disclaimers)
  const addFooter = (pageNum: number) => {
    ...
    // Disclaimer
    doc.text('⚠️ AI-Generated Content | Cross-Verify Facts | Not for Commercial Use', 105, 277, { align: 'center' });
    doc.setFontSize(6);
    doc.text('This is an experimental educational tool. AI can make mistakes. Not intended for exam preparation.', 105, 282, { align: 'center' });
    doc.text(`Generated: ${new Date().toLocaleDateString()} | Page ${pageNum}`, 105, 287, { align: 'center' });
  };
  ...
  doc.text(data.location, 105, yPos, { align: 'center' });

  yPos += 15;
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(0, 0, 0);
  const vibeLines = doc.splitTextToSize(`"${data.vibe}"`, 160);
  doc.text(vibeLines, 105, yPos, { align: 'center' });

  yPos += 20;
  // Rating badge
  doc.setFillColor(255, 215, 0); // Gold
  doc.circle(105, yPos, 12, 'F');
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(`${data.rating}/10`, 105, yPos + 4, { align: 'center' });
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  const descLines = doc.splitTextToSize((data.description || '').replace(/\*\*/g, ''), 170);
  ...
  if (data.geographicalContext) {
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('GEOGRAPHICAL CONTEXT', 20, yPos);
    yPos += 8;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text(`State: ${data.geographicalContext.state || 'N/A'}`, 20, yPos);
    yPos += 5;
    doc.text(`Main River: ${data.geographicalContext.mainRiver || 'N/A'}`, 20, yPos);
    yPos += 5;
    doc.text(`Climate: ${data.geographicalContext.climate || 'N/A'}`, 20, yPos);
    yPos += 5;
    doc.text(`Elevation: ${data.geographicalContext.elevation || 'N/A'}`, 20, yPos);
    yPos += 10;
  }
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('HISTORICAL SIGNIFICANCE', 20, yPos);
  yPos += 10;
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  const historyText = (data.history || '').replace(/\*\*/g, '');
  const historyLines = doc.splitTextToSize(historyText, 170);
  doc.text(historyLines, 20, yPos);
  yPos += historyLines.length * 5 + 10;

  if (data.importantTimeline && yPos < 250) {
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('Important Timeline:', 20, yPos);
    yPos += 7;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    data.importantTimeline.forEach((item) => {
      doc.text(`• ${item.year}: ${item.event}`, 25, yPos);
      yPos += 5;
    });
  }
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('ARCHITECTURE & DESIGN', 20, yPos);
  yPos += 10;
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  const archText = (data.architecture || '').replace(/\*\*/g, '');
  const archLines = doc.splitTextToSize(archText, 170);
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('CULTURAL PRACTICES', 20, yPos);
  yPos += 10;

  if (data.culturalPractices) {
    doc.setFontSize(11);
    doc.text('Festivals:', 20, yPos);
    yPos += 6;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    data.culturalPractices.regionalFestivals?.forEach((f) => {
      doc.text(`• ${f}`, 25, yPos);
      yPos += 5;
    });
    yPos += 5;
    ...
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text('Regional Cuisine:', 20, yPos);
    yPos += 6;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    data.culturalPractices.regionalCuisine?.forEach((f) => {
      doc.text(`• ${f}`, 25, yPos);
      yPos += 5;
    });
    yPos += 5;
    ...
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text('Local Handicrafts:', 20, yPos);
    yPos += 6;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    data.culturalPractices.localHandicrafts?.forEach((f) => {
      doc.text(`• ${f}`, 25, yPos);
      yPos += 5;
    });
  }
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('DID YOU KNOW?', 20, yPos);
  yPos += 10;
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  data.trivia?.forEach((t) => {
    const triviaLines = doc.splitTextToSize(`• ${t}`, 170);
    doc.text(triviaLines, 20, yPos);
    yPos += (triviaLines.length * 5) + 2;
  });
  ...
  if (data.nearbyPlaces) {
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    data.nearbyPlaces.forEach((p) => {
      const placeLines = doc.splitTextToSize(`• ${p}`, 170);
      doc.text(placeLines, 20, yPos);
      yPos += (placeLines.length * 5) + 2;
    });
  }
  ...
  // Save
  doc.save(`${data.title.replace(/[^a-z0-9]/gi, '_')}_Report.pdf`);
};
